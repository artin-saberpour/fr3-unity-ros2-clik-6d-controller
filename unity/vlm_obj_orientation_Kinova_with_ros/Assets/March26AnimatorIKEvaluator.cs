...
public class March26AnimatorIKEvaluator : MonoBehaviour
{
    [Header("Input")]
    ...
    [Header("Output")]
    public string outputRelativePath = "trajectory/march26_animator_ik_comfort";
    ...
    [ContextMenu("Export March26 Animator IK Comfort")]
    ...
    private IEnumerator ExportBatchCoroutine()
    {
        ...
        string inputDir = Path.Combine(Application.streamingAssetsPath, folderRelativeToStreamingAssets);
        string outputDir = Path.Combine(Application.dataPath, outputRelativePath);

        if (!Directory.Exists(inputDir))
        {
            Debug.LogError("Input folder not found: " + inputDir);
            yield break;
        }

        Directory.CreateDirectory(outputDir);

        var files = Directory.GetFiles(inputDir, fileExtension, SearchOption.TopDirectoryOnly)
        ...
        foreach (var path in files)
        {
            ...
            if (string.IsNullOrWhiteSpace(activeObject) || string.IsNullOrWhiteSpace(relativeHand))
            {
            ...
            if (!TryExtractPoseFromFrame(lastFrame, activeObject, out PoseData objectPose))
            {
                File.WriteAllText(outPath, "NaN");
                continue;
            }
            ...
            Vector3 after = objectPose.position + afterTargetOffset;
...
private void SaveOverallAverageFile(string outputDir, List<Result> results)
{
    ...
    private static string ExtractRelativeHandFromFilename(string filename)
    {
        ...
        string token = parts[1].ToLowerInvariant();
