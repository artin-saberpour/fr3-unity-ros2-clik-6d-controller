...
class ee_to_joint_inverse_kinematics(Node):
    ...
    #     JOINTS = [
    #         ("fr3_joint1", (0, 0, 1), (0, 0, 0.333)),
    #         ("fr3_joint2", (0, 0, 1), (0, 0, 0)),
    #         ("fr3_joint3", (0, 0, 1), (0, 0, 0.316)),
    #         ("fr3_joint4", (0, 0, 1), (0.0825, 0, 0)),
    #         ("fr3_joint5", (0, 0, 1), (-0.0825, 0.384, 0)),
    #         ("fr3_joint6", (0, 0, 1), (0, 0, 0)),
    #         ("fr3_joint7", (0, 0, 1), (0.088, 0, 0)),
    #     ]

    #     for name, axis, offset in JOINTS:
    ...
    #         # segment with zero frame (rotation only)
    #         chain.addSegment(
    #             PyKDL.Segment(name + "_rot", joint, PyKDL.Frame.Identity())
    #         )
    ...
    #                 name + "_link",
    #                 PyKDL.Joint(PyKDL.Joint()),
    #                 PyKDL.Frame(PyKDL.Vector(*offset))
    #             )
    #         )
    ...
    def build_chain_from_urdf_4(self, robot: URDF, base: str, tip: str):
        child_to_joint = {}
        ...
        while cur != base:
            if cur not in child_to_joint:
                raise RuntimeError(f"Cannot find parent joint for link '{cur}'.")
            j = child_to_joint[cur]
            joints_reversed.append(j)
            cur = j.parent
        ...
        for j in joints:
            ...
            if j.origin:
                if j.origin.xyz:
                    xyz = tuple(map(float, j.origin.xyz))
                if j.origin.rpy:
                    rpy = tuple(map(float, j.origin.rpy))

            R = rpy_to_kdl(rpy[0], rpy[1], rpy[2])
            p = PyKDL.Vector(*xyz)
            origin_frame = PyKDL.Frame(R, p)

            # --- URDF axis (joint origin_frame) ---
            if j.axis is None:
                raise RuntimeError(f"Joint {j.name} has no axis!")

            axis_urdf = PyKDL.Vector(
                float(j.axis[0]),
                float(j.axis[1]),
                float(j.axis[2]),
            )

            # ✅ ROTATE AXIS INTO SEGMENT FRAME
            axis_segment = R * axis_urdf
            ...
            seg = PyKDL.Segment(j.child, kdl_joint, origin_frame)
    ...
    def build_chain_from_urdf_backup3(self, robot: URDF, base: str, tip: str):
        ...
        for j in joints:
            ...
            # --- Joint origin (parent link → joint frame)
            xyz = (0.0, 0.0, 0.0)
            rpy = (0.0, 0.0, 0.0)
            if j.origin:
                if j.origin.xyz:
            ...
            axis = (0.0, 0.0, 1.0)
            if j.axis:
                axis = normalize(tuple(map(float, j.axis)))
            axis_vec = PyKDL.Vector(*axis)

            # 1) Joint segment (NO translation)
            kdl_joint = PyKDL.Joint(
                j.name,
                PyKDL.Vector.Zero(),
                axis_vec,
            ...
            joint_seg = PyKDL.Segment(
                j.name + "_joint",
                kdl_joint,
                PyKDL.Frame.Identity()
            )
            chain.addSegment(joint_seg)

            # 2) Fixed link offset segment (NO joint)
            link_seg = PyKDL.Segment(
                j.child,
                PyKDL.Joint(PyKDL.Joint()),
                origin_frame   # this is the URDF origin xyz+rpy
            )
            chain.addSegment(link_seg)
    ...
    def build_chain_from_urdf_backup2(self, robot: URDF, base: str, tip: str):
        # Map child link → joint
        child_to_joint = {}
        for j in robot.joints:
            if j.child:
    ...
    def build_chain_from_urdf_backup1(self, robot: URDF, base: str, tip: str):
        ...
        while cur != base:
            if cur not in child_to_joint:
                raise RuntimeError(f"Cannot find parent joint for link '{cur}'. Check base_link/tip_link names.")
            j = child_to_joint[cur]
            joints_reversed.append(j)
            cur = j.parent
        ...
        for j in joints:
            if j.type not in ["revolute", "continuous"]:
                # You can extend this later; for FR3 it’s revolute/continuous.
                continue

            joint_names.append(j.name)

            # URDF joint origin (transform from parent link to joint frame)
            xyz = (0.0, 0.0, 0.0)
            rpy = (0.0, 0.0, 0.0)
            if j.origin is not None:
                if j.origin.xyz is not None:
            ...
            R = rpy_to_kdl(rpy[0], rpy[1], rpy[2])
            p = PyKDL.Vector(xyz[0], xyz[1], xyz[2])
            frame = PyKDL.Frame(R, p)

            # URDF axis is expressed IN JOINT FRAME
            if j.name == "fr3_joint2":
                axis = (1.0, 0.0, 0.0)
                print("assigned joint2 axis")
            else:
                axis = (0.0, 0.0, 1.0)
            ...
            axis_vec_joint = PyKDL.Vector(axis[0], axis[1], axis[2])

            # KDL joint axis must be expressed in the segment frame.
            # Because we set the segment's frame = joint origin (parent->joint),
            # the joint axis in that frame is exactly axis_vec_joint.
            kdl_joint = PyKDL.Joint(j.name, PyKDL.Vector.Zero(), axis_vec_joint, PyKDL.Joint.RotAxis)

            seg = PyKDL.Segment(j.child, kdl_joint, frame)
            chain.addSegment(seg)
